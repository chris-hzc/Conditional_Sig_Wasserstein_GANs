...
def run(algo_id, base_config, base_dir, dataset, spec, data_params={}):
    ...
    #for i in range(5):
    #    x_fake = create_summary(dataset, base_config.device, algo.G, base_config.p, 3000, x_real_test, one=True)
    #    sss=np.array(x_fake[0,:,0])
    #    log_return=sss*std+mu
    #    log_return = np.exp(log_return)
    #    log_return=np.cumprod(log_return)
    #    plt.plot(lislog_return)
    #plt.show()
